...
* **Artist enrichment** — map artist names to their Spotify/YouTube/Mixcloud/Bandcamp/SoundCloud pages once, and every synced event that features them gets those links and embeds automatically.
* **Sold out and canceled handling** — sold-out events are dimmed, events with CANCELED in the title are struck through, and past events sort below upcoming ones with a divider.
...
EventMesh connects to external services to import and enrich event data. No data is sent anywhere unless you configure a source or provider mapping.
...
When a synced event links to an artist page or track on one of these providers (from Holvi data or your own artist map under EventMesh → Settings), the plugin requests that provider's public oEmbed endpoint to fetch embeddable player markup for the URL in question. Only that provider URL is sent; no visitor or site data is included. See each provider's terms and privacy policy: [Spotify](https://www.spotify.com/legal/), [YouTube](https://www.youtube.com/t/terms), [Mixcloud](https://www.mixcloud.com/terms/), [Bandcamp](https://bandcamp.com/terms_of_use), [SoundCloud](https://soundcloud.com/terms-of-use).
...
4. Go to **EventMesh → Settings** to choose the sync interval and (optionally) map artists to their media provider pages.
...
Connectors are self-contained: a directory under `src/Connectors/` with a `register.php` that hooks `eventmesh/register_connectors`. The bundled Dummy connector (Settings → Test connector) is a minimal reference implementation.
...
2. Sources page with Holvi shop URLs.
3. The event list block in the editor.
4. Settings: sync interval, artist map, and source management.
...
* Artist-to-provider mapping with oEmbed enrichment.
* Sold-out dimming, CANCELED strike-through, past/upcoming ordering.
* Source disable/enable with reversible archiving.
* Diagnostics page with background sync health.
* Optional test connector with sample data.
